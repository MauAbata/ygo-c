...
ygo_bin_errno_t ygo_bin_read_int(ygo_bin_read_context_t *ctx, uintmax_t *dest, size_t n) {
    ...
    for (size_t i = 0; i < n; i++) {
        ret <<= 8u;
        if (ctx->buffer != NULL) {
            uint8_t byte = ctx->buffer[ctx->ptr];
            ret |= byte;
        }
        ctx->ptr++;
    }
...
ygo_bin_errno_t ygo_bin_read_enum(ygo_bin_read_context_t *ctx, int *dest, size_t n) {
    uintmax_t ret = 0;
    ygo_bin_errno_t err = ygo_bin_read_int(ctx, &ret, n);
    *dest = ret;
    return err;
}
...
ygo_bin_errno_t ygo_bin_read_int8(ygo_bin_read_context_t *ctx, uint8_t *dest) {
    uintmax_t ret = 0;
    ygo_bin_errno_t err = ygo_bin_read_int(ctx, &ret, 1);
    *dest = ret;
    return err;
}
...
ygo_bin_errno_t ygo_bin_read_int16(ygo_bin_read_context_t *ctx, uint16_t *dest) {
    uintmax_t ret = 0;
    ygo_bin_errno_t err = ygo_bin_read_int(ctx, &ret, 2);
    *dest = ret;
    return err;
}
...
ygo_bin_errno_t ygo_bin_read_int32(ygo_bin_read_context_t *ctx, uint32_t *dest) {
    uintmax_t ret = 0;
    ygo_bin_errno_t err = ygo_bin_read_int(ctx, &ret, 4);
    *dest = ret;
    return err;
}
...
void ygo_bin_write_int(ygo_bin_write_context_t *ctx, const uint32_t value, size_t n) {
    if (ctx == NULL) return;
    for (size_t i = n; i > 0; i--) {
        uint8_t byte = value >> ((i - 1) * 8);
        if (ctx->buffer != NULL) ctx->buffer[ctx->ptr] = byte;
        ctx->ptr++;
    }
}
...
void ygo_bin_write_record_header(ygo_bin_write_context_t *ctx, ygo_bin_record_header_t *header) {
    ...
    ygo_bin_write_int(ctx, header->record_type, 1);
    ygo_bin_write_int(ctx, header->data_version, 1);
...
void ygo_bin_write_record_end(ygo_bin_write_context_t *ctx) {
    ...
    ygo_bin_write_int(ctx, crc16, 2);
    ygo_bin_write_int(ctx, 0x0000, 2); // unused data.
}
